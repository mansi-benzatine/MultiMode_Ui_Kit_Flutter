...
class ExploreStockScreen extends StatefulWidget {
  ...
  final bool isCandleGraphVisualView;

  static Route<void> route({
    required WishListItemData data,
    bool isCandleGraphVisualView = false,
    int currentIndex = 0,
  }) {
    return MaterialPageRoute(
      builder: (_) => ExploreStockScreen(
        data: data,
        isCandleGraphVisualView: isCandleGraphVisualView,
        currentIndex: currentIndex,
      ),
    );
  }

  const ExploreStockScreen({
    super.key,
    required this.data,
    this.currentIndex = 0,
    this.isCandleGraphVisualView = false,
  });
...
class _ExploreStockScreenState extends State<ExploreStockScreen> with SingleTickerProviderStateMixin implements TopBarClickListener {
  ...
  void initState() {
    ...
    showCandleChart = widget.isCandleGraphVisualView;
    _fillData();
  }
