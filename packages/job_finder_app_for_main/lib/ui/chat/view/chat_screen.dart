...
class ChatScreen extends StatefulWidget {
  ...
  static Route<void> route({required String userName, required String userProfile, required String lastMessage}) {
  ...
  const ChatScreen({super.key, required this.userName, required this.userProfile, required this.lastMessage});
...
class _ChatScreenState extends State<ChatScreen> implements TopBarClickListener {
  ...
  void fillData() {
    ...
    if (widget.lastMessage.isNotEmpty) {
      final firstMessage = chatList.isNotEmpty ? chatList.first.message.trim() : '';
      if (widget.lastMessage.trim() != firstMessage) {
        chatList.insert(
          0,
          ChatMessage(
  ...
  void onTopBarClick(String name, {bool value = true}) {
    ...
    if (name == Constant.strVideoCall) {
      Navigator.push(context, VideoCallScreen.route(userName: widget.userName));
    }
    if (name == Constant.strVoiceCall) {
      Navigator.push(context, VoiceCallScreen.route(userProfile: widget.userProfile, userName: widget.userName));
    }
  }
}
...
class ChatBubble extends StatelessWidget {
  ...
  const ChatBubble({super.key, required this.message, required this.userProfile});
