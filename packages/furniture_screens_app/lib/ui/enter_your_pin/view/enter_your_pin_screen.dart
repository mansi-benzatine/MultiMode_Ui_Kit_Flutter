...
class EnterYourPinScreen extends StatefulWidget {
  final bool isForOrderSuccessDialog;

  static Route<void> route({bool isForOrderSuccessDialog = false}) {
    return MaterialPageRoute(builder: (_) => EnterYourPinScreen(isForOrderSuccessDialog: isForOrderSuccessDialog));
  }

  const EnterYourPinScreen({super.key, this.isForOrderSuccessDialog = false});
...
class _EnterYourPinScreenState extends State<EnterYourPinScreen> implements TopBarClickListener {
  ...
  void initState() {
    ...
    super.initState();
    if (widget.isForOrderSuccessDialog) {
