import 'package:bitx_app/ui/onboarding/on_boarding_screen.dart';
...
class BitXApp extends StatefulWidget {
  final ThemeData? externalThemeData;
  static final navigatorKey = GlobalKey<NavigatorState>();

  static void setLocale(BuildContext context, Locale newLocale) {
    _BitXAppState? state = context.findAncestorStateOfType<_BitXAppState>();
  ...
  const BitXApp({super.key, this.externalThemeData});

  @override
  State<BitXApp> createState() => _BitXAppState();
}

class _BitXAppState extends State<BitXApp> {
  Locale? _locale;
    ...
    return ScreenUtilInit(
      ...
      builder: (_, __) {
        return AnnotatedRegion<SystemUiOverlayStyle>(
          ...
          child: MaterialApp(
            navigatorKey: BitXApp.navigatorKey,
...
class BitXAppWrapper extends StatelessWidget {
  final ThemeData themeData;

  const BitXAppWrapper({super.key, required this.themeData});
    ...
    return Theme(
      ...
      child: PopScope(
        ...
        onPopInvokedWithResult: (didPop, result) {
          if (didPop) return;
          final navigator = BitXApp.navigatorKey.currentState;
        ...
        child: BitXApp(externalThemeData: themeData),
      ),
    );
  }
}
