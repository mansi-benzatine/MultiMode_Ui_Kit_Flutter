...
class UpdateDrivingDetailsScreen extends StatefulWidget {
  static Route<void> route() {
    return MaterialPageRoute(builder: (_) => const UpdateDrivingDetailsScreen());
  }

  const UpdateDrivingDetailsScreen({super.key});

  @override
  State<UpdateDrivingDetailsScreen> createState() => _UpdateDrivingDetailsScreenState();
}

class _UpdateDrivingDetailsScreenState extends State<UpdateDrivingDetailsScreen> implements TopBarClickListener {
...
class _UploadFileViewState extends State<UploadFileView> {
  final List<PlatformFile> _selectedFiles = [
    PlatformFile(
      name: 'dummy_bank.png',
      size: 0,
      path: AppAssets.imgDummyDrivingDetail,
      bytes: null,
    ),
    PlatformFile(
      name: 'dummy_bank.png',
      size: 0,
      path: AppAssets.imgDummyDrivingLicence,
      bytes: null,
    ),
  ];
          ...
          if (_selectedFiles.isNotEmpty)
            Wrap(
              ...
              children: _selectedFiles.map((file) {
                if (file.extension == 'jpg' || file.extension == 'jpeg' || file.extension == 'png' || file.extension!.startsWith('packages/')) {
                  return Stack(
                    ...
                    children: [
                      Container(
                        ...
                        child: ClipRRect(
                          ...
                          child: file.path!.startsWith('packages/')
                              ? Image.asset(
                                  file.path!,
                                )
                              : Image.file(
