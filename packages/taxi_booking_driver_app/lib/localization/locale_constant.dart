...
Future<Locale> setLocale(String languageCode) async {
  getIt
      ...
      .setString(LocalStorageService.selectedLanguage, languageCode);
  return _locale(languageCode);
}

Locale getLocale() {
  String languageCode = getIt.get<LocalStorageService>().getString(
...
Locale _locale(String languageCode) {
  return languageCode.isNotEmpty
      ? Locale(languageCode, '')
      : const Locale(Constant.languageEn, '');
}

void changeLanguage(BuildContext context, String selectedLanguageCode) async {
  var locale = await setLocale(selectedLanguageCode);
  if (context.mounted) {
    TaxiBookingDriverApp.setLocale(context, locale);
  }
}
