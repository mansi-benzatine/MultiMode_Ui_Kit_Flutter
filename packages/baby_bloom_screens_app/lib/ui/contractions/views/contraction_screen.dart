...
import 'package:baby_bloom_screens_app/ui/contractions/datamodel/contraction_data.dart';
...
class ContractionScreen extends StatefulWidget {
  final bool isForHistoryView;

  static Route<void> route({bool isForHistoryView = false}) {
    return MaterialPageRoute(builder: (_) => ContractionScreen(isForHistoryView: isForHistoryView));
  }

  const ContractionScreen({super.key, this.isForHistoryView = false});
...
class _ContractionScreenState extends State<ContractionScreen> implements TopBarClickListener {
  ...
  void initState() {
    ...
    super.initState();
    if (widget.isForHistoryView) {
      contractions = [Contraction(time: DateTime.now(), duration: const Duration(seconds: 55), interval: "1 min 35 sec")];
    }
  }
