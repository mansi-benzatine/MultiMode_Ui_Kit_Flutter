...
class SearchJobScreen extends StatefulWidget {
  ...
  final bool isFromSearchResult;
  final bool isFromNoResultFound;
  const SearchJobScreen({super.key, required this.isFromSearchJob, required this.isFromSearchResult, required this.isFromNoResultFound});
  static Route<void> route({
    required bool isFromSearchJob,
    required bool isFromSearchResult,
    required bool isFromNoResultFound,
  }) {
    return MaterialPageRoute(
        builder: (_) => SearchJobScreen(
              isFromNoResultFound: isFromNoResultFound,
              isFromSearchJob: isFromSearchJob,
...
class _SearchJobScreenState extends State<SearchJobScreen> implements TopBarClickListener {
  ...
  void initState() {
    super.initState();
    if (widget.isFromSearchResult) {
    } else if (widget.isFromNoResultFound) {
    } else if (widget.isFromSearchJob) {}
  }
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  IgnorePointer(
                    ignoring: true,
                    child: _SearchAndFilterView(
                      recentSearch: searchList,
                      recentSearches: recentSearchList,
                      recentViewed: recentlyViewedList,
                      isFromNoResultFound: widget.isFromNoResultFound,
                      isFromSearchResult: widget.isFromSearchResult,
                      isFromSearchJob: widget.isFromSearchJob,
                      defaultSearch: widget.isFromSearchResult ? "Designer" : null,
                    ),
                  ),
                ],
              ),
...
class _SearchAndFilterViewState extends State<_SearchAndFilterView> {
  ...
  void initState() {
    ...
    if (widget.isFromSearchResult && widget.defaultSearch != null) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isFromNoResultFound) {
      content = const _EmptySearchView();
    } else if (widget.isFromSearchResult) {
      content = _filteredJobs.isNotEmpty ? _SearchedView(recentViewList: _filteredJobs) : const _EmptySearchView();
    } else if (widget.isFromSearchJob) {
      content = _SearchingView(
              ...
              Expanded(
                child: Padding(
                  ...
                  child: CommonTextFormFieldWithPrefix(
                    ...
                    focusNode: widget.isFromSearchResult || widget.isFromNoResultFound ? AlwaysDisabledFocusNode() : _focusNode,
